...
import { StockWithdrawalReportDialog, WithdrawalReportData } from './stock-withdrawal-report-dialog';

export interface SelectedWithdrawalItem {
  item: InventoryItem;
  quantityToRemove: number;
}

interface StockWithdrawalDialogProps {
  open: boolean;
  onClose: () => void;
  stores: StoreSimple[];
...
export function StockWithdrawalDialog({
  open,
  onClose,
  stores,
  currentStoreId,
  onSuccess,
}: StockWithdrawalDialogProps): React.JSX.Element {
  ...
  const [withdrawalItems, setWithdrawalItems] = React.useState<SelectedWithdrawalItem[]>([]);
  const [searchValue, setSearchValue] = React.useState<InventoryItem | null>(null);
  const [searchInputValue, setSearchInputValue] = React.useState<string>('');
  const [submitting, setSubmitting] = React.useState<boolean>(false);
  const [errorMessage, setErrorMessage] = React.useState<string | null>(null);
  const [generatedReport, setGeneratedReport] = React.useState<WithdrawalReportData | null>(null);
  ...
  const fetchStoreInventory = React.useCallback(async (storeId: number) => {
    ...
    try {
      ...
      console.error('Error fetching inventory for store withdrawal:', err);
  ...
  // Update current stock in withdrawal items list when storeInventory refreshes
  React.useEffect(() => {
    if (storeInventory.length > 0 && withdrawalItems.length > 0) {
  ...
  const availableOptions = React.useMemo(() => {
    const selectedIds = new Set(withdrawalItems.map((ti) => ti.item.productVariantId));
    return storeInventory.filter((item) => !selectedIds.has(item.productVariantId));
  }, [storeInventory, withdrawalItems]);

  const handleAddProduct = (itemToAdd: InventoryItem | null) => {
    if (!itemToAdd) return;

    const existingIndex = withdrawalItems.findIndex(
      (ti) => ti.item.productVariantId === itemToAdd.productVariantId
    );

    if (existingIndex >= 0) {
      setErrorMessage(`El producto "${itemToAdd.description}" ya está en la lista de retiro.`);
      setSearchValue(null);
      setSearchInputValue('');
      return;
    }
    ...
    // Default 1 unit to remove
    setWithdrawalItems((prev) => [
      ...prev,
      { item: itemToAdd, quantityToRemove: 1 },
    ]);
      ...
      prev.map((ti) => {
        if (ti.item.productVariantId === productVariantId) {
          if (isNaN(rawVal) || rawVal < 1) {
            return { ...ti, quantityToRemove: 1 };
          }
          return { ...ti, quantityToRemove: rawVal };
        }
        return ti;
      })
    );
  ...
  const handleRemoveProduct = (productVariantId: string) => {
    setWithdrawalItems((prev) => prev.filter((ti) => ti.item.productVariantId !== productVariantId));
  };

  const isFormValid = React.useMemo(() => {
    if (withdrawalItems.length === 0) return false;
    for (const ti of withdrawalItems) {
      if (ti.quantityToRemove <= 0) return false;
    }
    return true;
  }, [withdrawalItems]);
  ...
  const handleSendWithdrawal = async () => {
    ...
    try {
      const payload = {
        ...
        items: withdrawalItems.map((ti) => ({
          productVariantId: ti.item.productVariantId,
          quantity: ti.quantityToRemove,
        })),
      };

      const res = await apiClient.post('/Inventory/bulk-withdraw', payload);

      const msg = `Retiro registrado exitosamente en ${storeName}`;
      onSuccess(msg);
      ...
      console.error('Stock withdrawal failed:', err);
      const serverMsg = err?.response?.data?.message || err?.message || 'Ocurrió un error al retirar el inventario.';
  ...
  const handleGenerateFormat = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet('Retiro de Inventario');
      ...
      const targetItems =
        withdrawalItems.length > 0
          ? withdrawalItems.map((i) => ({ item: i.item, qty: i.quantityToRemove || 0 }))
      ...
      saveAs(blob, `Formato_Retiro_Inventario_${storeNameStr}_${dateStr}.xlsx`);
    } catch (err) {
  ...
  const handleImportFormat = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const newWithdrawalItemsMap = new Map<string | number, SelectedWithdrawalItem>();
      // Retain existing items if any
      withdrawalItems.forEach((existing) => {
        newWithdrawalItemsMap.set(existing.item.productVariantId || existing.item.id, existing);
      });
      ...
      worksheet.eachRow((row, rowNumber) => {
        ...
        if (matchedItem) {
          ...
          newWithdrawalItemsMap.set(key, {
            item: matchedItem,
            quantityToRemove: qtyNum,
          });
          importedCount++;
        }
      });

      const updatedList = Array.from(newWithdrawalItemsMap.values());
      setWithdrawalItems(updatedList);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
          ...
          <Stack direction="row" alignItems="center" spacing={1.5}>
            ...
            <Box>
              <Typography variant="h6" fontWeight={700}>
                Retirar Productos de Inventario
              </Typography>
              <Typography variant="caption" color="text.secondary">
                Selecciona la sucursal de origen y registra la salida de unidades de stock.
              </Typography>
            </Box>
          </Stack>
        ...
        <Stack spacing={3}>
          ...
          <Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: 'background.neutral' }}>
            <FormControl fullWidth size="small">
              <InputLabel id="withdrawal-store-label">🏢 Sucursal / Tienda de Origen</InputLabel>
              <Select
                labelId="withdrawal-store-label"
                value={selectedStoreId}
                label="🏢 Sucursal / Tienda de Origen"
              ...
              <FormHelperText>Sucursal de donde se retirará el inventario</FormHelperText>
            </FormControl>
          </Paper>
          ...
          {/* List of Products to Withdraw */}
          <Box>
            <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: 1.5 }}>
              <Typography variant="subtitle2" fontWeight={700}>
                Productos a Retirar ({withdrawalItems.length})
              </Typography>
              <Stack direction="row" spacing={1}>
                ...
                {withdrawalItems.length > 0 && (
                  <Button size="small" color="secondary" onClick={() => setWithdrawalItems([])}>
                    Limpiar Lista
                  </Button>
                )}
              </Stack>
              ...
              >
                <Stack spacing={1} alignItems="center">
                  ...
                  <Typography variant="caption" color="text.disabled">
                    Selecciona productos en el buscador superior para registrarlos en el retiro.
                  </Typography>
                </Stack>
              </Paper>
            ) : (
                ...
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow sx={{ '& th': { fontWeight: 700, bgcolor: 'background.neutral' } }}>
                      ...
                      <TableCell align="center" sx={{ width: 140 }}>Cantidad a retirar</TableCell>
                      <TableCell align="center" sx={{ width: 50 }}></TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {withdrawalItems.map(({ item, quantityToRemove }) => {
      ...
      <DialogActions sx={{ p: 2.5, justifyContent: 'space-between' }}>
        ...
        >
          {submitting ? 'Guardando...' : 'Retirar de inventario'}
        </Button>
      </DialogActions>

      {/* Report Summary Modal */}
      <StockWithdrawalReportDialog
